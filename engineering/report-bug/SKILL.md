...
Check the existing `issues.md` registry and `issues/pending/` files:
...
**Numbering:** Read the `# Current Max Issue ID` section at the bottom of `docs/issues/issues.md`. The value there is the highest issue number ever assigned. The new issue number is `max + 1`. If the section or file does not exist, fall back to scanning `issues/pending/` and `issues/done/` for the highest existing number.

<bug-issue-template>
## Parent PRD

`docs/issues/<prd-filename>` (e.g., `docs/issues/prd-email.md`) or "None — standalone bug" if unrelated to any PRD.
...
- `docs/issues/pending/NNN-title.md` (if any)
...
Append a new row to `issues/issues.md` (or create the file if it doesn't exist):
