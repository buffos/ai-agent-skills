...
- **`issues/prd.md`** — the main reference PRD. Contains the overall problem statement, solution summary, cross-cutting concerns (shared architecture, deployment, multi-account, AI integration, testing philosophy), links to sub-PRDs, and development phases.
- **`issues/prd-<feature>.md`** — one sub-PRD per feature domain (e.g., `prd-youtube.md`, `prd-email.md`, `prd-feeds.md`). Each sub-PRD is **self-contained**: it has its own user stories, implementation decisions, testing decisions, and out-of-scope section. A sub-PRD should be independently actionable — an agent working on that feature should not need to read the main PRD except for cross-cutting patterns.
...
Before anything else, check if a main PRD (`issues/prd.md`) already exists:

- **If it exists:** Ask the user which sub-PRD file to write to (e.g., `issues/prd-payments.md`). The new feature will be written as a sub-PRD and linked from the main PRD.
- **If it does NOT exist:** You will need to create both:
  1. A main PRD (`issues/prd.md`) with the overall problem statement, solution, cross-cutting architecture, and a "Feature Sub-Documents" section linking to sub-PRDs.
  2. A sub-PRD (`issues/prd-<feature>.md`) with the feature-specific content.
...
**For the sub-PRD** (`issues/prd-<feature>.md`), use the sub-PRD template below. Create the `issues/` directory if it doesn't exist. Do NOT submit a GitHub issue or call any external service.

**If the main PRD does not exist**, also create `issues/prd.md` using the main PRD template below. If the main PRD already exists, update it to add a link to the new sub-PRD in the "Feature Sub-Documents" section.
