---
name: write-a-prd
description: Generate or refresh the PRD for a bounded capability as part of the exact specification workflow. Use when the user specifically needs PRD authoring or PRD refresh for a selected capability; prefer the architecture specification pipeline for the full bounded-node workflow.
---
...
This skill is now a focused PRD step, not the full bounded-node tightening workflow. When the user needs the full exact-spec path for a bounded capability, prefer the architecture suite starting with `bounded-capability-spec-orchestrator`.

You may skip steps if they are unnecessary, but do not invent missing planning structure when the graph should exist.

If `.okf/` exists, use `$okf-planning-profile` and read [../okf-planning-profile/REFERENCE.md](../okf-planning-profile/REFERENCE.md) before making assumptions about capability-node fields, planning states, or graph relationships.
...
If the selected capability is still broad or mixes multiple outcomes, stop and route back to `fog-of-war-planning` rather than forcing a PRD.

If the capability is bounded but not exact enough, route into the architecture specification pipeline rather than improvising a partial PRD.
