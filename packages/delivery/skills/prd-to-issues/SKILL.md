...
If `.okf/` exists, use `$okf-planning-profile` and read [../../../planning/skills/okf-planning-profile/REFERENCE.md](../../../planning/skills/okf-planning-profile/REFERENCE.md) before inferring capability ownership, shared-concern links, or issue write-back behavior.
...
For each approved slice, write a markdown file in `docs/agents/issues/pending/` using the naming pattern `docs/agents/issues/pending/NNN-short-title.md` (e.g. `docs/agents/issues/pending/001-add-user-auth.md`).

**Numbering:** Read the `# Current Max Issue ID` section at the bottom of `docs/agents/issues/issues.md`. The value there is the highest issue number ever assigned. Start numbering new issues from `max + 1`. If the section or file does not exist, fall back to scanning `docs/agents/issues/pending/` and `docs/agents/issues/done/` for the highest existing number.
...
`docs/agents/issues/prd.md` (or whichever PRD file was used)
...
- Blocked by `docs/agents/issues/pending/NNN-title.md` (if any)
...
After all issue files are created, create or update an `docs/agents/issues/issues.md` file in the same directory as the PRD (e.g., `docs/agents/issues/issues.md`).
