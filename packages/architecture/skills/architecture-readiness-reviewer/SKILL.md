...
If `.okf/` exists, review the artifact set for one owning capability node. Use
the node, its scope, and its linked shared concerns as the baseline for what
...
- when `.okf/` exists, update the owning capability node's `readiness_review` reference after writing
