...
If `.okf/` exists, start from the owning capability node and linked shared
concerns. Keep the model scoped to that capability rather than pretending to
model the whole product unless the selected node is the whole product.
...
- when `.okf/` exists, update the owning capability node's `domain_model` reference after writing
