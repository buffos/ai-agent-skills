...
This skill is primarily for pre-OKF or pre-bounded work. Once a capability is
already `bounded`, prefer `requirements-gap-analyzer` unless the bounded node
still lacks basic discovery facts.
...
- when `.okf/` exists, update the owning capability node's `discovery_notes` reference after writing
