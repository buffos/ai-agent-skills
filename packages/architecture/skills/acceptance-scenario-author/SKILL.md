...
If `.okf/` exists, keep scenarios scoped to the owning capability node and the
shared concerns that affect its externally visible behavior.
...
- when `.okf/` exists, update the owning capability node's `scenarios` reference after writing
