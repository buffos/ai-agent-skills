...
If `.okf/` exists, derive the use-case model from the owning capability node,
its scoped PRD, and linked shared concerns rather than from global repo
artifacts alone.
...
- when `.okf/` exists, update the owning capability node's `use_cases` reference after writing
