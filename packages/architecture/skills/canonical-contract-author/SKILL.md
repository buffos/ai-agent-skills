...
If `.okf/` exists, keep the contract scoped to the owning capability node and
its shared concerns. Do not let one node's contract silently absorb unrelated
product surface.
...
- when `.okf/` exists, update the owning capability node's `contract` reference after writing
