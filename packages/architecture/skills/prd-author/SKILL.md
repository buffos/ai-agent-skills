...
If `.okf/` exists, this skill is downstream of the owning capability node. Read
the node first, keep the PRD scoped to that node, and write the resulting PRD
...
- when `.okf/` exists, update the owning capability node's `prd` reference after writing
