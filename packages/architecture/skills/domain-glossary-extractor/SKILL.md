...
If `.okf/` exists, scope the glossary to the owning capability and any linked
shared concerns that materially affect its language.
...
- when `.okf/` exists, update the owning capability node's `glossary` reference after writing
