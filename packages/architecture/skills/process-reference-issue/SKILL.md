...
If `.okf/` exists, resolve the owning capability node before implementation,
read its linked artifacts, and write issue progress back into the node when the
work is complete.

Treat the registry as the routing source of truth. Before implementation,
verify that the selected issue row and the issue file agree on:
...
Prefer the explicit `Issue File` path from the registry row. If the file is not
found there, stop and inform the user.
...
If the issue metadata conflicts with the registry row, stop and fix the issue
tracking state before implementing code.
...
- update the `Issue File` cell to the `issues/done/YYYYMMDD-NNN-short-title.md` path
...
- when `.okf/` exists, refreshed the owning capability node's `issues` references and any related progress notes

On a blocked run:

- no false completion
- clear explanation of the blocker
- updated `docs/architecture/<capability-slug>/requirements-gap-analysis.md` if the blocker is a true spec gap
