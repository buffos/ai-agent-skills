...
In the new flow, this is the normal clarification step for a `bounded`
capability. If `.okf/` exists, scope the analysis to the owning capability node
and linked shared concerns.
...
- when `.okf/` exists, update the owning capability node's `gap_analysis` reference after writing
