...
- `bounded` -> run `$grill-with-docs` to tighten language and decisions
- `specified` -> route to `$write-a-prd`
...
- `grill-with-docs` on the bounded node
- direct PRD handoff
...
Treat `.okf/` as the map and `docs/agents/issues/` as the delivery layer.
